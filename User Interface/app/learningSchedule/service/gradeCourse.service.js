...
var gradeCourseService = /** @class */ (function () {
    function gradeCourseService(_http) {
        this._http = _http;
    }
    gradeCourseService.prototype.getGradeCourses = function () {
        return this._http.get("http://localhost:5570/api/GradeCourse/GetGradeCourses")
            .map(function (response) { return response.json(); });
    };
    ;
    gradeCourseService.prototype.saveGradeCourse = function (GradeCourse) {
        var bodyString = JSON.stringify(GradeCourse); // Stringify payload
    ...
    ;
    gradeCourseService.prototype.updateGradeCourse = function (GradeCourse) {
        var bodyString = JSON.stringify(GradeCourse); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    gradeCourseService.prototype.deleteGradeCourse = function (GradeCourse) {
        var bodyString = JSON.stringify(GradeCourse); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    gradeCourseService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [http_1.Http])
    ], gradeCourseService);
    return gradeCourseService;
}());
exports.gradeCourseService = gradeCourseService;
//# sourceMappingURL=gradeCourse.service.js.map