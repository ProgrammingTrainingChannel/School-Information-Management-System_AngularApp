...
var campusService = /** @class */ (function () {
    ...
    campusService.prototype.getCampuses = function () {
        return this._http.get("http://localhost:5570/api/Campus/GetCampuss")
            .map(function (response) { return response.json(); });
    };
    ;
    campusService.prototype.saveCampus = function (campus) {
        var bodyString = JSON.stringify(campus); // Stringify payload
    ...
    ;
    campusService.prototype.updateCampus = function (campus) {
        var bodyString = JSON.stringify(campus); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    campusService.prototype.deleteCampus = function (campus) {
        var bodyString = JSON.stringify(campus); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
...
//# sourceMappingURL=grading.SubmitGrade.service.js.map