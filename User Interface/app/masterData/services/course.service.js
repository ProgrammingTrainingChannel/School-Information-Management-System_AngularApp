...
var courseService = /** @class */ (function () {
    ...
    courseService.prototype.saveCourse = function (course) {
        var bodyString = JSON.stringify(course); // Stringify payload
        ...
        return this._http.post("http://localhost:5570/api/Course/SaveCourse", bodyString, options) // ...using post request
    ...
    courseService.prototype.updateCourse = function (course) {
        ...
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
        return this._http.post("http://localhost:5570/api/Course/UpdateCourse", bodyString, options) // ...using post request
            .map(function (res) { return res.json(); }) // ...and calling .json() on the response to return data
            .catch(function (error) { return Observable_1.Observable.throw(error.json().error || 'Server error'); }); //...errors if any
    };
    ;
    courseService.prototype.deleteCourse = function (course) {
        ...
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
        return this._http.post("http://localhost:5570/api/Course/DeleteCourse", bodyString, options) // ...using post request
            .map(function (res) { return res.json(); }) // ...and calling .json() on the response to return data
            .catch(function (error) { return Observable_1.Observable.throw(error.json().error || 'Server error'); }); //...errors if any
    };
    ;
