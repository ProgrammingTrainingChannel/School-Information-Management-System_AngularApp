...
var studentTutorialService = /** @class */ (function () {
    function studentTutorialService(_http) {
        this._http = _http;
    }
    studentTutorialService.prototype.getStudentTutorials = function () {
        return this._http.get('http://localhost:5570/api/StudentTutorial/GetStudentTutorials')
            .map(function (response) { return response.json(); });
    };
    ;
    studentTutorialService.prototype.saveStudentTutorial = function (teacherEvaluation) {
    ...
    ;
    studentTutorialService.prototype.updateStudentTutorial = function (teacherEvaluation) {
        var bodyString = JSON.stringify(teacherEvaluation); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    studentTutorialService.prototype.deleteStudentTutorial = function (teacherEvaluation) {
        var bodyString = JSON.stringify(teacherEvaluation); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    studentTutorialService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [http_1.Http])
    ], studentTutorialService);
    return studentTutorialService;
}());
exports.studentTutorialService = studentTutorialService;
//# sourceMappingURL=studentTutorial.service.js.map