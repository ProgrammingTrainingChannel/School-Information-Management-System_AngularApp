...
var academicQuarterService = /** @class */ (function () {
    function academicQuarterService(_http) {
        this._http = _http;
    }
    academicQuarterService.prototype.getAcademicQuarters = function () {
        return this._http.get('http://localhost:5570/api/AcademicQuarter/GetAcademicQuarters')
            .map(function (response) { return response.json(); });
    };
    ;
    academicQuarterService.prototype.saveAcademicQuarter = function (academicQuarter) {
        var bodyString = JSON.stringify(academicQuarter); // Stringify payload
    ...
    ;
    academicQuarterService.prototype.updateAcademicQuarter = function (academicQuarter) {
        var bodyString = JSON.stringify(academicQuarter); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    academicQuarterService.prototype.deleteAcademicQuarter = function (academicQuarter) {
        var bodyString = JSON.stringify(academicQuarter); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    academicQuarterService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [http_1.Http])
    ], academicQuarterService);
    return academicQuarterService;
}());
exports.academicQuarterService = academicQuarterService;
//# sourceMappingURL=academicQuarter.service.js.map