import { Injectable } from '@angular/core';
import { Http, Response } from '@angular/http';
import { Headers, RequestOptions } from '@angular/http';
import { Observable } from 'rxjs/Observable';

import 'rxjs/add/operator/map';
import 'rxjs/add/operator/catch';

import { IGradeSection } from '../interface/gradeSection.interface';

@Injectable()
export class gradeSectionService {

    constructor(private _http: Http) { }

    getGradeSections(): Observable<IGradeSection[]> {
        return this._http.get('http://localhost:5570/api/GradeSection/GetGradeSections')
            .map((response: Response) => <IGradeSection[]>response.json());
    };

    getGradeSectionsByGrade(gradeID: number): Observable<IGradeSection[]> {
        return this._http.get('http://localhost:5570/api/GradeSection/GetGradeSectionsByGrade?gradeID=' + gradeID)
            .map((response: Response) => <IGradeSection[]>response.json());
    };

    saveGradeSection(gradeSection: IGradeSection): Observable<any> {
        let bodyString = JSON.stringify(gradeSection); // Stringify payload
        let headers = new Headers({ 'Content-Type': 'application/json'}); // ... Set content type to JSON
        let options = new RequestOptions({ headers: headers }); // Create a request option

        return this._http.post("http://localhost:5570/api/GradeSection/SaveGradeSection", bodyString, options) // ...using post request
            .map((res: Response) => res.json()) // ...and calling .json() on the response to return data
            .catch((error: any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
    };

    updateGradeSection(woreda: IGradeSection): Observable<any> {
        let bodyString = JSON.stringify(woreda); // Stringify payload
        let headers = new Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        let options = new RequestOptions({ headers: headers }); // Create a request option

        return this._http.post("http://localhost:5570/api/GradeSection/UpdateGradeSection", bodyString, options) // ...using post request
            .map((res: Response) => res.json()) // ...and calling .json() on the response to return data
            .catch((error: any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
    };

    deleteGradeSection(woreda: IGradeSection): Observable<any> {
        let bodyString = JSON.stringify(woreda); // Stringify payload
        let headers = new Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        let options = new RequestOptions({ headers: headers }); // Create a request option

        return this._http.post("http://localhost:5570/api/GradeSection/DeleteGradeSection", bodyString, options) // ...using post request
            .map((res: Response) => res.json()) // ...and calling .json() on the response to return data
            .catch((error: any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
    };

}
