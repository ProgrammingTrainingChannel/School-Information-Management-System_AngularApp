...
var examTypeService = /** @class */ (function () {
    function examTypeService(_http) {
        this._http = _http;
    }
    examTypeService.prototype.getExamTypes = function () {
        return this._http.get('http://localhost:5570/api/ExamType/GetExamTypes')
            .map(function (response) { return response.json(); });
    };
    ;
    examTypeService.prototype.saveExamType = function (examType) {
        var bodyString = JSON.stringify(examType); // Stringify payload
    ...
    ;
    examTypeService.prototype.updateExamType = function (examType) {
        var bodyString = JSON.stringify(examType); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    examTypeService.prototype.deleteExamType = function (examType) {
        var bodyString = JSON.stringify(examType); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
