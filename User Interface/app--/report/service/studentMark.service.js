...
var studentMarkService = /** @class */ (function () {
    function studentMarkService(_http) {
        this._http = _http;
    }
    studentMarkService.prototype.getStudentMarks = function () {
        return this._http.get("http://localhost:5570/api/StudentMark/GetStudentMarks")
            .map(function (response) { return response.json(); });
    };
    ;
    studentMarkService.prototype.saveStudentMark = function (studentMark) {
        var bodyString = JSON.stringify(studentMark); // Stringify payload
    ...
    ;
    studentMarkService.prototype.updateStudentMark = function (studentMark) {
        var bodyString = JSON.stringify(studentMark); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    studentMarkService.prototype.deleteStudentMark = function (studentMark) {
        var bodyString = JSON.stringify(studentMark); // Stringify payload
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        var options = new http_2.RequestOptions({ headers: headers }); // Create a request option
    ...
    ;
    studentMarkService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [http_1.Http])
    ], studentMarkService);
    return studentMarkService;
}());
exports.studentMarkService = studentMarkService;
//# sourceMappingURL=studentMark.service.js.map